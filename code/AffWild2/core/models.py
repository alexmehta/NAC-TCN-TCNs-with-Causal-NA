...
class ABAW3Model(LightningModule):
    ...
    def __init__(self):
        ...
        if cfg.MODEL_TYPE == 'gru':
            ...
            self.fc_aux_tranf = nn.Linear(
                self.num_feats['feat'], self.num_feats['feat'])
            ...
            self.fc = nn.Linear(
                self.num_feats['feat'] + cfg.GRU.HIDDEN_SIZE * (cfg.GRU.BIDIRECTIONAL + 1), self.num_outputs)
        else:
