""" Adapted from: HuynhVanThong & NguyenHong Hai, Department of AI Convergence, Chonnam Natl. Univ.
Modified from GRU to LSTM by Alexander Mehta
"""
import os
...
class ABAWLSTM(LightningModule):
    ...
    def __init__(self):
        ...
        if cfg.MODEL_TYPE == 'gru':
            ...
            self.fc = nn.Sequential(nn.Linear(
                (cfg.GRU.HIDDEN_SIZE * (cfg.GRU.BIDIRECTIONAL + 1)), self.num_outputs), nn.Tanh())
            ...
            self.gru=    nn.LSTM(input_size=self.num_feats['feat'],
                       hidden_size=cfg.GRU.HIDDEN_SIZE,
                       num_layers=cfg.GRU.NUM_LAYERS, batch_first=True,
                       dropout=cfg.GRU.DROPOUT,
                       bidirectional=cfg.GRU.BIDIRECTIONAL)
            ...
            self.fc_gru = nn.Linear(
                cfg.GRU.HIDDEN_SIZE * (cfg.GRU.BIDIRECTIONAL + 1), self.num_outputs)

            self.fc = nn.Linear(cfg.GRU.HIDDEN_SIZE *
                                (cfg.GRU.BIDIRECTIONAL + 1), self.num_outputs)
        else:
        ...
        if cfg.MODEL.BACKBONE_PRETRAINED != '':
            ...
            pretrained_static = torch.load(
                cfg.MODEL.BACKBONE_PRETRAINED)['state_dict']
    ...
    def forward(self, batch):
        ...
        if cfg.MODEL_TYPE == 'gru':
            ...
            # batch size x seq_len x hidden size
        ...
        if cal_loss:
            if self.task != 'MTL':
                ...
                if cfg.COMBINE_LOSS:
                    ...
                    loss = loss_tranf_coeff * loss_tranf + \
                        (1-loss_tranf_coeff) * loss
        return out, loss
...
if __name__ == "__main__":
    model = ABAWLSTM().cuda()
    input = torch.rand(1,128,440).cuda()
