...
class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.2,local=14):
        ...
        self.conv2 = NeighborhoodAttention1D(n_outputs, kernel_size,
...
if __name__ == "__main__":
    ...
    model = TemporalConvNet(
        ...
        440, [16]*2, kernel_size=11, dropout=0.5, dilation_size=2).cuda()
    # with torch.cuda.device(0):


    #     macs, params = get_model_complexity_info(model, (440,128),as_strings=True,
    ...
    with torch.autograd.profiler.profile(use_cuda=True) as prof:
        model(torch.randn(1, 440, 128).cuda())
